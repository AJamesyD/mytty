...
struct SettingsView: View {
  private let config = MisttyConfig.load()
  ...
  var body: some View {
    Form {
      ...
      Section {
        HStack {
          Text(MisttyConfig.configFileURL.path)
            .foregroundStyle(.secondary)
            .font(.caption)
            .textSelection(.enabled)
          Spacer()
          Button("Open Config File") {
            NSWorkspace.shared.open(MisttyConfig.configFileURL)
          }
        }
      } header: {
        Text("Configuration File")
      } footer: {
        Text("Edit the TOML file directly to change settings. Restart Mistty to apply changes.")
          .font(.caption)
