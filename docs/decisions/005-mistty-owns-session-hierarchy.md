# ADR-005: Mistty Owns the Session Hierarchy
...
1. **Delegate to libghostty.** Less code, but Mistty becomes a skin over
   Ghostty with no control over session structure, persistence, or sidebar.

2. **Own the hierarchy.** Mistty manages SessionStore, sessions, tabs, and
   panes. libghostty only owns the rendering surface. More code, full control.

## Decision

Option 2. Mistty owns the full hierarchy:
...
layout is a recursive tree managed by Mistty's layout engine, not by
libghostty's built-in split model.

## Consequences

- The sidebar, session persistence, IPC, and custom split layouts are all
  possible because Mistty controls the data model.
- Future features (floating panes, saved layouts, daemon mode) build on
  this hierarchy without touching the terminal layer.
- Cost: Mistty implements tab and split management from scratch (creation,
  closing, reordering, focus tracking, split resize).
- The IPC layer operates on Mistty's model objects. The CLI can create
  sessions, manage tabs, and query pane state because Mistty owns these
  concepts.
...
Owning the session hierarchy is what makes Mistty a product rather than
