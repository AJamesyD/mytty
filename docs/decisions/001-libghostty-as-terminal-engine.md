...
Mytty needed a terminal rendering engine. Three options:
...
3. **Embed libghostty.** Use Ghostty's C library as a rendering engine.
   libghostty handles terminal parsing, rendering, and input. Mytty owns
...
per pane. Mytty creates surfaces, forwards input events, and receives
...
- Mytty ships a working terminal from day one. Development focuses on
...
- Mytty is tied to Ghostty's release cycle for terminal improvements.
...
"libghostty parses all escape sequences internally." Mytty handles typed
