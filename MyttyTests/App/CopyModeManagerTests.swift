...
final class CopyModeManagerTests: XCTestCase {
  ...
  func test_findMatch_forward_findsAfterCursor() {
    let result = manager.findMatchOnLine("hello world hello", query: "hello", cursorCol: 0, forward: true)
  ...
  func test_findMatch_forward_matchAtStart() {
    let result = manager.findMatchOnLine("hello world", query: "hello", cursorCol: -1, forward: true)
  ...
  func test_findMatch_reverse_findsBeforeCursor() {
    let result = manager.findMatchOnLine("hello world hello", query: "hello", cursorCol: 12, forward: false)
    XCTAssertEqual(result, 0)
  }

  func test_findMatch_reverse_noMatchBeforeCursor() {
    let result = manager.findMatchOnLine("hello world", query: "hello", cursorCol: 0, forward: false)
    XCTAssertNil(result)
  }

  func test_findMatch_reverse_multipleMatches() {
    let result = manager.findMatchOnLine("aaa bbb aaa bbb aaa", query: "aaa", cursorCol: 16, forward: false)
    XCTAssertEqual(result, 8)
  }

  func test_findMatch_reverse_caseInsensitive() {
    let result = manager.findMatchOnLine("HELLO world", query: "hello", cursorCol: 6, forward: false)
    XCTAssertEqual(result, 0)
  }
