...
final class IPCListener {
  ...
  private nonisolated static func acceptLoop(
    state: OSAllocatedUnfairLock<ListenerState>, service: MyttyIPCService, broker: EventBroker
  ) {
    while true {
  ...
  private nonisolated static func handleConnection(
    _ fd: Int32, service: MyttyIPCService, broker: EventBroker
  ) async {
    ...
    while true {
      ...
      guard let request = try? JSONDecoder().decode(JSONRPCMessage.Request.self, from: messageData)
        ...
        let errResp = JSONRPCMessage.Response.error(
          id: 0, code: MyttyIPC.JSONRPCErrorCode.parseError, message: "Parse error")
      ...
      case "initialize":
        let result: JSONValue = [
          "serverVersion": .string(MyttyIPC.protocolVersion),
  ...
  private nonisolated static func callService(
    ...
    do {
      ...
      let rpcCode: Int
      switch MyttyIPC.ErrorCode(rawValue: error.code) {
      case .entityNotFound: rpcCode = MyttyIPC.JSONRPCErrorCode.entityNotFound
      case .invalidArgument: rpcCode = MyttyIPC.JSONRPCErrorCode.invalidArgument
      case .notSupported: rpcCode = MyttyIPC.JSONRPCErrorCode.notSupported
      default: rpcCode = MyttyIPC.JSONRPCErrorCode.operationFailed
      }
