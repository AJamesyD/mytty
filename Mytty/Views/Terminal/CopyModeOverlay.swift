...
struct CopyModeOverlay: View {
  ...
  var body: some View {
    ZStack(alignment: .topLeading) {
      ...
      // Cursor
      Rectangle()
        .fill(MyttyTheme.copyModeCursor)
        ...
        HStack {
          if state.subMode == .searchForward || state.subMode == .searchReverse {
            ...
            Text(modeIndicatorText)
              ...
              .padding(.vertical, 2)
              .background(
                MyttyTheme.modeIndicatorBackground, in: RoundedRectangle(cornerRadius: 4))
          }
          Spacer()
        }
        .padding(4)
      ...
      if state.showingHelp {
        CopyModeHelpOverlay(cellHeight: cellHeight)
          .frame(maxWidth: .infinity, maxHeight: .infinity)
          .background(MyttyTheme.modalBackdrop)
      }
    }
...
struct SelectionHighlightView: View {
  ...
  private func drawCharacterWise(
    ...
    for row in visibleMin...visibleMax {
      ...
      context.fill(Path(rect), with: .color(MyttyTheme.selectionHighlight))
    }
  }
