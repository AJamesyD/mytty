...
final class SessionStore {
  ...
  func createDetachedSession(name: String, directory: URL) -> MyttySession {
    MyttySession(
      id: generateSessionID(),
      name: name,
      directory: directory,
  ...
  func restore(from state: PersistentState) {
    ...
    // TODO: createSession and MyttyTab.init each create a throwaway default
  ...
  private func restoreLayoutNode(_ node: PersistentLayoutNode, home: URL) -> (
    PaneLayoutNode, [MyttyPane]
  ) {
    let fm = FileManager.default
    switch node {
    case .leaf(let persistedPane):
      let pane = MyttyPane(id: generatePaneID())
      let dir =
